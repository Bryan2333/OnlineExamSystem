       ...
       <ul class="item">
         ...
         <li @click="flag = !flag">
           ...
           <div class="msg"  v-if="flag" @click="flag = !flag">
             ...
             <p>Admission ticket:  {{userInfo.id}}</p>
           </div>
         </li>
        ...
        <transition name="slider-fade">
          <div class="left" v-if="slider_flag">
            ...
            <div class="l-bottom">
              ...
              <div class="final" @click="commit()">Submit</div>
            </div>
          </div>
        </transition>
        <!--右边选择答题区-->
        ...
        <div class="right">
          <div class="title">
            ...
            <i class="iconfont icon-right auto-right"></i>
              <span>{{topicCount[0] + topicCount[1] + topicCount[2]}} questions in total  <i class="iconfont icon-time"></i>countdown：<b>{{time}}</b>minute</span>
          </div>
          <div class="content">
            ...
            <div v-if="currentType == 1">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li> <el-tag type="success">Correct posture：</el-tag><span class="right">{{reduceAnswer.rightAnswer}}</span></li>
                  <li><el-tag>Topic analysis：</el-tag></li>
                  <li>{{reduceAnswer.analysis == null ? 'No resolution yet': reduceAnswer.analysis}}</li>
                </ul>
              </div>
            </div>
            <div class="fill" v-if="currentType == 2">
              <div v-for="(item,currentIndex) in part" :key="currentIndex">
                <el-input placeholder="Please fill it out here"
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li> <el-tag type="success">Correct posture：</el-tag><span class="right">{{topic[2][index].answer}}</span></li>
                  <li><el-tag>Topic analysis：</el-tag></li>
            ...
            <div class="judge" v-if="currentType == 3">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li> <el-tag type="success">Correct posture：</el-tag><span class="right">{{topic[3][index].answer}}</span></li>
                  <li><el-tag>Topic analysis：</el-tag></li>
                  <li>{{topic[3][index].analysis == null ? 'No resolution yet': topic[3][index].analysis}}</li>
                </ul>
              </div>
            </div>
          </div>
          <div class="operation">
            <ul class="end">
              <li @click="previous()"><i class="iconfont icon-previous"></i><span>Previous question</span></li>
              <li @click="mark()"><i class="iconfont icon-mark-o"></i><span>mark</span></li>
              <li @click="next()"><span>Next question</span><i class="iconfont icon-next"></i></li>
            </ul>
          </div>
        </div>
        </transition>
  ...
  methods: {
    ...
    change(index) { //选择题
      ...
      if(this.index < len) {
        ...
        console.log(`Overall length${len}`)
        console.log(`current index:${index}`)
        this.title = "Please select the correct option"
    ...
    fill(index) { //填空题
      ...
      if(index < len) {
        if(index < 0) {
          ...
          console.log(`Overall length${len}`)
          console.log(`current index:${index}`)
          this.title = "Please fill in the answer in the horizontal line"
    ...
    judge(index) { //判断题
      ...
      if(this.index < len) {
        if(this.index < 0){
          ...
          this.title = "Please make the right judgment"
    ...
    commit() { //答案提交计算分数
      ...
      console.log(`Current total score${finalScore}`)
      if(this.time != 0) {
        this.$confirm("If the end of the exam has not arrived, whether to submit the paper in advance","Friendly reminder",{
          confirmButtonText: 'Submit your papers now',
          cancelButtonText: 'Check again',
          type: 'warning'
        }).then(() => {
          console.log("Submit the papers")
          let date = new Date()
          ...
          console.log("Continue answering the question")
        })
      }
    },
    showTime() { //倒计时
      setInterval(() => {
        ...
        if(this.time == 10) {
          this.$message({
            showClose: true,
            type: 'error',
            message: 'Note that there are 10 minutes left in the exam!!'
          })
          if(this.time == 0) {
            console.log("The time for the exam has arrived, and it is mandatory to hand in the papers.")
          }
        }
      },1000 * 60)
    }
