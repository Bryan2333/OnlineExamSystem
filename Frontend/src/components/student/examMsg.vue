  ...
  <div id="msg">
    ...
    <div class="wrapper">
      <ul class="top">
        ...
        <li class="right">
          <div>
            <span class="count">Full Marks  {{score[0] + score[1] + score[2] }}</span>
            <!-- <span class="score">{{ }}</span> -->
          </div>
        </li>
      </ul>
      <ul class="bottom">
        <li>Updated {{ examData.examDate }}</li>
        <li>From {{ examData.institute }}</li>
        <li class="btn">{{ examData.type }}</li>
        <li class="right">
          <el-button @click="toAnswer(examData.examCode)">Start Exam</el-button>
        </li>
      </ul>
      <ul class="info">
        ...
        <el-collapse-item class="header" name="0">
          <template slot="title" class="stitle">
            <div class="title">
              ...
              <span class="time">{{ examData.totalScore }}分 / {{ examData.totalTime }}minute</span>
              <el-button type="primary" size="small">Click here to view the question details</el-button>
            </div>
          </template>
          <el-collapse class="inner">
            <el-collapse-item>
              <template slot="title" name="1">
                <div class="titlei">Multiple choice questions ({{ topicCount[0] }} in total, Totaling{{ score[0] }})</div>
              </template>
              <div class="contenti">
            ...
            <el-collapse-item>
              <template slot="title" name="2">
                <div class="titlei">Fill-in-the-blank questions ({{ topicCount[1] }} in total,  Totaling{{ score[1] }})</div>
              </template>
              <div class="contenti">
            ...
            <el-collapse-item>
              <template slot="title" name="3">
                <div class="titlei">True/False ({{ topicCount[2] }} in total, Totaling {{ score[2] }})</div>
              </template>
              <div class="contenti">
